...
class SzyfrowanieCezara {
    ...
    string tablicaZnakowDoSzyfrowaniaMaleLiter = "abcdefghijklmnoprstuwyz";
    string tablicaZnakowDoSzyfrowaniaDuzeLiter = "ABCDEFGHIJKLMNOPRSTUWYZ";
...
public:
    ...
    string ZaSzyfruj(string tekst, int przesuniecie) {
        int iloscZnakowDoPrzeszukaniaWTablicyZnakow = tablicaZnakowDoSzyfrowaniaMaleLiter.length(); //Przypisanie dlugosci tablicy znakow
        int iloscZnakowDoPrzeszukaniaWCiagu = ciagDoZaszyfrowania.length(); //Przypisanie dlugosci tekstu do zaszyfrowania
        int prawdziwePrzesuniecie = 0; //Przesuniecie
        ciagZaszyfrowany = ciagDoZaszyfrowania; //Przekopiowanie tekstu wprowadzonego do zmiennej "Zaszyfrowanej", z tym ze jeszcze nie jest zaszyfrowana - robimy to zeby wszystkie znaki ktore nie beda zaszyfrowane byly na swoim miejscu
        for (int i = 0; i < iloscZnakowDoPrzeszukaniaWCiagu; i++) { //Pierwsza petla leci dla kazdego znaku z naszego Tekstu do zaszyfrowania
            for (int j = 0; j < iloscZnakowDoPrzeszukaniaWTablicyZnakow; j++) { // Druga petla leci po kazdym mozluwym znaku aby mozliwe bylo zaszyfrowanie - dla litery z tekstu szukamy odpowiedniego indeksu w tablicy znakow
                if (ciagDoZaszyfrowania[i] == tablicaZnakowDoSzyfrowaniaMaleLiter[j]) { // Przeszukiwanie malych liter - jesli je znajdziemy idziemy dalej do kodu
                    if (j + przesuniecieSzyfru >= iloscZnakowDoPrzeszukaniaWTablicyZnakow) { //Tutaj sprawdzamy czy czasem nasze przesuniecie nie wyjdzie poza zakres tablicy znakow tzn np. mamy 'z' i przesuniecie o 3. My wiemy ze to bedzie 'c' ale program bez tego sprawdzenia nie wie
                        prawdziwePrzesuniecie = (j + przesuniecieSzyfru) % iloscZnakowDoPrzeszukaniaWTablicyZnakow; // Tutaj nadajemy przesuniecie w sposob taki ze jesli mialo by wyjsc poza zakres to bierzemy sobie reszte z dzielenia naszego przesuniecia + indeksu naszego znaku znalezionego i dlugosci maksymalnej tablicy co daje nam wartosc od poczatku tablicy
                        //cout << prawdziwePrzesuniecie;
                        ciagZaszyfrowany[i] = tablicaZnakowDoSzyfrowaniaMaleLiter[prawdziwePrzesuniecie]; // Przypisanie nowego znaku na stara pozycje
                        break;
                    }
                    else {
                        prawdziwePrzesuniecie = j + przesuniecieSzyfru; // Tutaj po prostu sumujemy bo wiemy ze nie wyjdziemy poza zakres
                        ciagZaszyfrowany[i] = tablicaZnakowDoSzyfrowaniaMaleLiter[prawdziwePrzesuniecie];
                        break;
                    }
                }
                else if (ciagDoZaszyfrowania[i] == tablicaZnakowDoSzyfrowaniaDuzeLiter[j]) { // Przeszukiwanie duzych liter
                    if (j + przesuniecieSzyfru >= iloscZnakowDoPrzeszukaniaWTablicyZnakow) {
                        ...
                        //cout << prawdziwePrzesuniecie;
                        ciagZaszyfrowany[i] = tablicaZnakowDoSzyfrowaniaDuzeLiter[prawdziwePrzesuniecie];
                        break;
                    }
                    else {
        ...
        return ciagZaszyfrowany; //Zwracamy ciag
    }
